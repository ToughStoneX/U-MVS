...
class MVSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        forward_flow_list = []
        backward_flow_list = []
        src_forward_flow_list = []
        src_backward_flow_list = []
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                for k in range(i+1, len(view_ids)):
                    forward_flow, backward_flow = self.load_flow(scan, vid, view_ids[k])
                    src_forward_flow_list.append(forward_flow)
                    src_backward_flow_list.append(backward_flow)
        ...
        src_forward_flow_list = np.stack(src_forward_flow_list)  # [(V-1+1)*(V-1)/2, 2, 512, 640]
        src_backward_flow_list = np.stack(src_backward_flow_list)  # [(V-1+1)*(V-1)/2, 2, 512, 640]
        ...
        return {"imgs": images,
                ...
                "src_forward_flow": src_forward_flow_list,
                "src_backward_flow": src_backward_flow_list,

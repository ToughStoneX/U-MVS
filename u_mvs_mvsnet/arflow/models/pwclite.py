...
# from models.correlation_naive import Correlation
...
class PWCLite(nn.Module):
    def __init__(self, cfg):
        ...
        self.corr = Correlation(pad_size=self.search_range, kernel_size=1, max_displacement=self.search_range,
        ...
        self.context_networks = ContextNetwork((self.flow_estimators.feat_dim + 2) * (self.n_frames - 1))
    ...
    def forward(self, x, with_bk=False):
        ...
        res_dict = {}
        if n_frames == 2:
            res_dict['flows_fw'] = self.forward_2frames(x[0], x[1])
            if with_bk:
                res_dict['flows_bw'] = self.forward_2frames(x[1], x[0])
        elif n_frames == 3:
            flows_10, flows_12 = self.forward_3frames(x[0], x[1], x[2])
            ...
            if with_bk:
                flows_32, flows_34 = self.forward_3frames(x[2], x[3], x[4])
    ...
    def forward_2frames(self, x1_pyramid, x2_pyramid):
        flows = []
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            # print('x_intm: {} flow: {}'.format(x_intm.shape, flow.shape))
    ...
    def forward_3frames(self, x0_pyramid, x1_pyramid, x2_pyramid):
        # outputs
        flows = []

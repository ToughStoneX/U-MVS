...
def train_sample(sample, detailed_summary=False):
    ...
    image_outputs = {"depth_est": depth_est, "depth_gt": sample["depth"].squeeze(dim=-1),
                     "ref_img": sample["imgs"][:, 0], "mask": sample["mask"],
...
def test_sample(sample, detailed_summary=True):
    ...
    scalar_outputs = {"loss": loss, "loss1": criterion.loss1, "loss2": criterion.loss2,
                      "reconstr_loss": criterion.reconstr_loss, "ssim_loss": criterion.ssim_loss,
                      "smooth_loss": criterion.smooth_loss}
    image_outputs = {"depth_est": depth_est, "depth_gt": sample["depth"].squeeze(dim=-1),

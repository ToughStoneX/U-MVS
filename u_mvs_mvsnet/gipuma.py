...
def mvsnet_to_gipuma(scan_folder, scan, dtu_test_root, gipuma_point_folder):
    image_folder = os.path.join(dtu_test_root, scan, 'images')
    cam_folder = os.path.join(dtu_test_root, scan, 'cams')
    ...
    for view in range(0, 49):
        ...
        out_cam_file = os.path.join(gipuma_cam_folder, "{:08d}.jpg.P".format(view))
    ...
    for view in range(0, 49):
        ...
        out_image_file = os.path.join(gipuma_image_folder, "{:08d}.jpg".format(view))
...
def probability_filter(dense_folder, prob_threshold):
    ...
    depth_folder = os.path.join(dense_folder, 'depth_est')
    confidence_folder = os.path.join(dense_folder, 'confidence')

    # convert cameras
    # image_names = os.listdir(image_folder)
    image_names = os.listdir(confidence_folder)
...
def gipuma_filter(testlist, outdir, prob_threshold, disp_threshold, num_consistent, fusibile_exe_path, dtu_test_root):
    ...
    for scan in testlist:
        ...
        mvsnet_to_gipuma(dense_folder, scan, dtu_test_root, point_folder)

...
def save_depth(img_wh=(1920, 1056)):
    ...
    # (self, datapath, split='intermediate', nviews=3, img_wh=(1920, 1056)， ndepths=192):
    # test_dataset = MVSDataset(args.testpath, args.split, args.num_view, (1920, 1056), args.numdepth)
    test_dataset = MVSDataset(args.testpath, args.split, args.num_view, img_wh, args.numdepth)
    # test_dataset = MVSDataset(args.testpath, args.split, args.num_view)
    TestImgLoader = DataLoader(test_dataset, args.batch_size, shuffle=False, num_workers=4, drop_last=False)

    # model
    model = CascadeMVSNetUnc(refine=False, ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
...
if __name__ == '__main__':
    ...
    if args.split == "intermediate":
        ...
        for scan in scans:
            ...
            scan_ply_path = os.path.join(args.outdir, scan + '.ply')
            ...
            filter_depth(scan_folder, out_folder, os.path.join(args.outdir, scan + '.ply'), 
                args.geo_pixel_thres, args.geo_depth_thres, photo_thres[scan], img_wh, image_sizes[scan], geo_mask_thres[scan], args.num_view, scan) 
    # advanced dataset

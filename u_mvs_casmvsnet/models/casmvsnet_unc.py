...
class CostRegNetManualDrop(nn.Module):
    def __init__(self, in_channels, base_channels):
        super(CostRegNetManualDrop, self).__init__()
    ...
    def forward(self, x, drop_flag=False):
        conv0 = self.conv0(x)
...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization, prob_volume_init=None, drop_flag=False):
        ...
        cost_reg = cost_regularization(volume_variance, drop_flag=drop_flag)
...
class CascadeMVSNetUnc(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
        ...
        super(CascadeMVSNetUnc, self).__init__()
        self.refine = refine
        ...
        self.feature = FeatureNetUnc(base_channels=8, stride=4, num_stage=self.num_stage, arch_mode=self.arch_mode)
        if self.share_cr:
            # self.cost_regularization = CostRegNet(in_channels=self.feature.out_channels, base_channels=8)
            self.cost_regularization = CostRegNetManualDrop(in_channels=self.feature.out_channels, base_channels=8)
        else:
            # self.cost_regularization = nn.ModuleList([CostRegNet(in_channels=self.feature.out_channels[i],
            #                                                      base_channels=self.cr_base_chs[i])
            #                                           for i in range(self.num_stage)])
            self.cost_regularization = nn.ModuleList([CostRegNetManualDrop(in_channels=self.feature.out_channels[i],
                                                                 base_channels=self.cr_base_chs[i])
                                                      for i in range(self.num_stage)])
        if self.refine:
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for nview_idx in range(imgs.size(1)):  # imgs shape (B, N, C, H, W)
            img = imgs[:, nview_idx]
            if nview_idx == 0:
                feature, outputs_var = self.feature(img, ref_flag=True)  # ref view
            else:
                feature, _ = self.feature(img, ref_flag=False)  # src views
        ...
        for stage_idx in range(self.num_stage):
            ...
            if stage_idx == 0:  # 在第一个阶段就加dropout，其他地方就不加了
                drop_flag = True
            else:
                drop_flag = False
            ...
            outputs["var{}".format(stage_idx + 1)] = outputs_var["var{}".format(stage_idx + 1)]

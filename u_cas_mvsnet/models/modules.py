...
class FeatureNetUnc(nn.Module):
    """
    A small CNN is added to estimate the aleatoric uncertainty. 
    """
    def __init__(self, base_channels, num_stage=3, stride=4, arch_mode="unet"):
        super(FeatureNetUnc, self).__init__()
        assert arch_mode in ["unet", "fpn"], print("mode must be in 'unet' or 'fpn', but get:{}".format(arch_mode))
        print("*************feature extraction arch mode:{}****************".format(arch_mode))
        self.arch_mode = arch_mode
        self.stride = stride
        ...
        self.out1 = nn.Conv2d(base_channels * 4, base_channels * 4, 1, bias=False)
        self.out_channels = [4 * base_channels]
        self.unc_conv1 = nn.Sequential(
            Conv2d(base_channels * 4, 128, 3, 1, padding=1),
            Conv2d(128, 32, 3, 1, padding=1),
        ...
        if self.arch_mode == 'unet':
            if num_stage == 3:
                ...
                self.unc_conv2 = nn.Sequential(
                    Conv2d(base_channels * 2, 128, 3, 1, padding=1),
                    Conv2d(128, 32, 3, 1, padding=1),
                    nn.Conv2d(32, 1, 3, 1, padding=1)
                )
                self.unc_conv3 = nn.Sequential(
                    Conv2d(base_channels, 128, 3, 1, padding=1),
                    Conv2d(128, 32, 3, 1, padding=1),
                    nn.Conv2d(32, 1, 3, 1, padding=1)
                )
    ...
    def forward(self, x, ref_flag=False):
        conv0 = self.conv0(x)
        conv1 = self.conv1(conv0)
        conv2 = self.conv2(conv1)
        ...
        outputs["stage1"] = out
        if ref_flag:
            var = self.unc_conv1(out)
            outputs_var["var1"] = var
        if self.arch_mode == "unet":
            if self.num_stage == 3:
                intra_feat = self.deconv1(conv1, intra_feat)
                out = self.out2(intra_feat)
                outputs["stage2"] = out
                if ref_flag:
                    var2 = self.unc_conv2(out)
                    outputs_var["var2"] = var2

                intra_feat = self.deconv2(conv0, intra_feat)
                out = self.out3(intra_feat)
                outputs["stage3"] = out
                if ref_flag:
                    var3 = self.unc_conv3(out)
                    outputs_var["var3"] = var3
...
class CostRegNetDrop(nn.Module):
    """
    MC-Dropout is used to estimate the epistemic uncertainty. 
    """
    def __init__(self, in_channels, base_channels):
